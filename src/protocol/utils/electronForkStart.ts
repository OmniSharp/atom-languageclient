...
(function() {
    ...
    var fsWriteSyncString = function(fd: any, str: any, position: any, encoding: any) {
        //  fs.writeSync(fd, string[, position[, encoding]]);
        var buf = new Buffer(str, encoding || 'utf8');
        return fsWriteSyncBuffer(fd, buf, 0, buf.length);
    };

    var fsWriteSyncBuffer = function(fd: any, buffer: any, off: any, len: any) {
        off = Math.abs(off | 0);
        len = Math.abs(len | 0);
    ...
    fs.writeSync = function(fd: any, data: any, position: any, encoding: any) {
        if (fd !== 1) {
