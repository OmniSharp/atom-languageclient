...
function calcuateMovement(previous: Autocomplete.RequestOptions, current: Autocomplete.RequestOptions) {
    if (!current) {
        return { reset: true, current: current, previous: <Autocomplete.RequestOptions>undefined! };
    }
...
class CompletionProvider implements IDisposable {
    ...
    private previous: Autocomplete.RequestOptions;
    ...
    public getSuggestions(options: Autocomplete.RequestOptions): Promise<Autocomplete.Suggestion[]> {
        ...
        if (!this.results) {
            this.results = Omni.request(solution => solution.autocomplete(_.clone(autoCompleteOptions))).toPromise();
        }
